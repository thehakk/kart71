...
export function findCiftFinishPlan(
  ...
  for (const discard of ordered) {
    ...
    if (rest.length === 0) {
      if (allowDiscardOnly) return { pairs: [], discardCardId: discard.id };
      continue;
    }
    const greedy = tryGreedyPairCover(rest, taban);
    if (greedy) {
      return { pairs: greedy.pairs, discardCardId: discard.id };
    }
    const cover = coverHand(rest, taban, 'pairs');
    if (cover && cover.pairs.length > 0) {
      return { pairs: cover.pairs, discardCardId: discard.id };
    }
  }
  return null;
}
...
export function findPerFinishPlan(
  ...
  for (const discard of ordered) {
    const rest = hand.filter((c) => c.id !== discard.id);
    if (rest.length === 0) {
      if (allowDiscardOnly) return { melds: [], pairs: [], discardCardId: discard.id };
      continue;
    }
    const greedy = tryGreedyMeldCover(rest);
    if (greedy) {
      return {
        melds: greedy.melds,
        pairs: [],
    ...
    const cover = coverHand(rest, taban, 'melds');
    if (cover && cover.melds.length > 0) {
      return {
        melds: cover.melds,
        pairs: [],
        discardCardId: discard.id,
...
export function findFinishPlan(
  ...
  if (player.isCiftci || player.openType === 'cift') {
    return findCiftFinishPlan(hand, taban, allowDiscardOnly);
  }
  return (
    findPerFinishPlan(hand, taban, allowDiscardOnly) ??
    findCiftFinishPlan(hand, taban, allowDiscardOnly)
  );
}
